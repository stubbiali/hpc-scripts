...
def core(branch: str, env: defs.ProgrammingEnvironment, partition: defs.Partition) -> str:
    with common.utils.batch_file(filename="prepare_cloudsc_versions") as (f, fname):
        ...
        common.utils.export_variable("CLOUDSC_VERSIONS", project_dir)
        venv_dir = os.path.join(project_dir, "_venv", env)
        common.utils.export_variable("CLOUDSC_VERSIONS_VENV", venv_dir)
